(() => {
  ...
  // claude-chats: its README screens, replayed
  {
    const pre = $("#chats-term");
    const LIST = [
    ...
    const HELP = "j/k move · o open · p preview · d mark · D all · u none · / filter · Enter delete marked · q quit";
    const PREVIEW = [
    ...
    let cursor = 0, preview = false, s = 0, timer = 0;
    const mark = (row, i) => ` ${marked.has(i) ? '<span class="x">✗</span>' : row[1] === "●" ? '<span class="ok">●</span>' : " "} `;
    function render() {
      if (preview) {
        pre.innerHTML = PREVIEW.map((l, n) => {
          const [left, right] = l.split("│");
          if (right === undefined) return `<span class="dim">${esc(l)}</span>`;
          let lhs = esc(left);
          const row = n - 2;
          if (row >= 0 && row < LIST.length) {
            const body = left.slice(3).trimEnd();
            lhs = mark(left, row) + (row === cursor ? `<span class="sel">${esc(body)}</span>` : esc(body)) + " ".repeat(left.length - 3 - body.length);
          } else if (n === 0) {
            lhs = `<span class="dim">${esc(left)}</span>`;
          }
          const rhs = n === 0 ? `<span class="dim">${esc(right)}</span>` : esc(right).replace(/^( +)(claude|you)/, '$1<span class="pr">$2</span>');
          return `${lhs}<span class="dim">│</span>${rhs}`;
        }).join("\n");
        return;
      }
      const head = `<span class="dim">Claude Code chats  5 chats  ${marked.size} marked</span>`;
      const rows = LIST.map((r, i) => mark(r, i) + (i === cursor ? `<span class="sel">${esc(r.slice(3))}</span>` : esc(r.slice(3))));
      pre.innerHTML = [head, ...rows, `<span class="dim">${esc(HELP)}</span>`].join("\n");
    }
    // mark the two chats the README marks, then preview the first one
    const script = ["j", "d", "j", "j", "d", "k", "k", "k", "p", "·", "·", "·", "·", "p", "u"];
    const step = () => {
      const k = script[s++ % script.length];
      if (k === "j") cursor = Math.min(LIST.length - 1, cursor + 1);
      if (k === "k") cursor = Math.max(0, cursor - 1);
      if (k === "d") marked.has(cursor) ? marked.delete(cursor) : marked.add(cursor);
      if (k === "p") preview = !preview;
      if (k === "u") marked.clear();
      render();
    };
    render();
    demos["claude-chats"] = {
      start() { clearInterval(timer); if (!reduced) timer = setInterval(step, 720); },
      stop() { clearInterval(timer); },
    };
  }
